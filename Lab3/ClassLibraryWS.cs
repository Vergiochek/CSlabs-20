...
namespace ClassLibraryWS
{
  public class Pathes
   ...
   public class CryptOptions
    ...
    //Начало основного класса EtlXmlJsonOption
    public class EtlXmlJsonOption
    {
        ...
        public void Do(string filePath)
        {
            ...
            var newstr = copyOptions.Copystr(archstr, pathes.TargetDirectory);
            var newcrypt = archivizeOptions.DearchivizeCrypt(newstr);
            var getstr =cryptOptions.Encryptf(newcrypt);

            string folder = Path.Combine(pathes.TargetDirectory, pathes.ArchFolder);
